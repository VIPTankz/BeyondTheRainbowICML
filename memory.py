...
Transition_dtype_procgen = np.dtype([('timestep', np.int32), ('state', np.uint8, (64, 64)), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.bool_)])
blank_trans_procgen = (0, np.zeros((64, 64), dtype=np.uint8), 0, 0.0, False)
...
class SegmentTree():
  def __init__(self, size, procgen=False):
    self.index = 0
    self.size = size
    self.blank_transition = blank_trans_procgen if procgen else blank_trans
    self.transition_dtype = Transition_dtype_procgen if procgen else Transition_dtype
    self.full = False  # Used to track actual capacity
    self.tree_start = 2**(size-1).bit_length()-1  # Put all used node leaves on last tree level
    self.sum_tree = np.zeros((self.tree_start + self.size,), dtype=np.float32)
    self.data = np.array([self.blank_transition] * size, dtype=self.transition_dtype)  # Build structured array
...
class ReplayMemory():
  def __init__(self, capacity, n, discount, device, alpha, beta, procgen=False):
    self.device = device
    ...
    self.transitions = SegmentTree(capacity, procgen=procgen)  # Store transitions in a wrap-around cyclic buffer within a sum tree for querying priorities
